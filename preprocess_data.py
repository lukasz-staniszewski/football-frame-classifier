...
def create_csv_balanced(csv_in, csv_out, csv_size):
    ...
    for ind, cat in enumerate(list(scaled_weights.keys())):
        cat_df = df_in.loc[df_in['category'] == cat].sample(int(scaled_weights[cat]*csv_size), replace=True)
        if ind == 0:
...
def main(args):
    ...
    create_csv_balanced(csv_in = arg.input_csv, csv_out=arg.output_csv, csv_size=arg.size_csv)
