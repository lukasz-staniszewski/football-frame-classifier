...
def load_dl():
    ...
    loader = FramesDataLoader(
        images_folder=config["images_folder"],
        batch_size=config["batch_size"],
        csv_path=config["csv_path"],
        csv_path_tf=config["csv_path_tf"],
        shuffle=config["shuffle"],
        validation_split=config["validation_split"],
        num_workers=config["num_workers"],
        is_with_aug=config["is_with_aug"],
    )
...
def perform_metrics():
    ...
    correct_predicts_by_class = {
        name_class: 0 for name_class in all_class_names
    }
    predicts_by_class = {
        name_class: 0 for name_class in all_class_names
    }
