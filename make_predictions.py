...
def load_dl():
    ...
    loader = TestDataLoader(images_folder=test_folder, batch_size=config_dl["batch_size"], shuffle=False, validation_split=0.0,num_workers=config_dl["num_workers"])
    return loader
...
def perform_predicts():
    ...
    predictions_df = pd.DataFrame({"filename":paths,"category":map(lambda x: loader.index2class[x.item()],preds.int()), "probability":probs})
